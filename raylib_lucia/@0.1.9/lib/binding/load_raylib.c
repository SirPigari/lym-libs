...
static void (*fp_DrawModel)(Model, Vector3, float, Color) = NULL;
static void (*fp_DrawModelEx)(Model, Vector3, Vector3, float, Vector3, Color) = NULL;
static void (*fp_DrawModelWires)(Model, Vector3, float, Color) = NULL;
...
static void load_raylib() {
    ...
    fp_DrawModel = get_symbol(raylib_lib, "DrawModel");
    fp_DrawModelEx = get_symbol(raylib_lib, "DrawModelEx");
    fp_DrawModelWires = get_symbol(raylib_lib, "DrawModelWires");
...
RLAPI void DrawModel(Model model, Vector3 position, float scale, Color tint) { load_raylib(); if(fp_DrawModel) fp_DrawModel(model, position, scale, tint); }
RLAPI void DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint) { load_raylib(); if(fp_DrawModelEx) fp_DrawModelEx(model, position, rotationAxis, rotationAngle, scale, tint); }
RLAPI void DrawModelWires(Model model, Vector3 position, float scale, Color tint) { load_raylib(); if(fp_DrawModelWires) fp_DrawModelWires(model, position, scale, tint); } 
